...
class POSTokenizer(object):
    ...
    def __init__(self, tokenizer=None):
        self.tokenizer = tokenizer or example_cut.Tokenizer()
...
dt = POSTokenizer(example_cut.dt)
...
def cut(sentence, HMM=True):
    ...
    """
    global dt
    if example_cut.pool is None:
        for w in dt.cut(sentence, HMM=HMM):
            yield w
    else:
        parts = strdecode(sentence).splitlines(True)
        if HMM:
            result = example_cut.pool.map(_lcut_internal, parts)
        else:
            result = example_cut.pool.map(_lcut_internal_no_hmm, parts)
        for r in result:
            for w in r:
                yield w
