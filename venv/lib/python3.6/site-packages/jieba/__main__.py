...
parser.add_argument("-V", '--version', action='version',
                    version="Jieba " + example_cut.__version__)
parser.add_argument("filename", nargs='?', help="input file")

args = parser.parse_args()

if args.quiet:
    example_cut.setLogLevel(60)
if args.pos:
    import example_cut.posseg
    posdelim = args.pos
    def cutfunc(sentence, _, HMM=True):
        for w, f in example_cut.posseg.cut(sentence, HMM):
            yield w + posdelim + f
else:
    cutfunc = example_cut.cut
...
if args.dict:
    example_cut.initialize(args.dict)
else:
    example_cut.initialize()
if args.user_dict:
    example_cut.load_userdict(args.user_dict)
